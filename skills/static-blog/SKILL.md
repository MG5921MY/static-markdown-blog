...
**展示序与阅读序解耦**（勿再理解为「一份 order 同时决定列表和上下篇」）：

| 语义 | 由谁决定 | 用途 |
|------|----------|------|
| **展示序** | `content.sort`（含 order） | 首页 / 分类 / tree 组内 / 分页列表 |
| **阅读序** | 同一组 `by` 字段，**强制 asc** | 上一篇 / 下一篇：键值更小 = 上一篇 |
| **归档时间线** | 日期降序 + 同日 `readingIndex` | 归档页、404「最近文章」 |

`content.sort` 数组有序（首项主排序，后续项依次打平局）：
...
content:
  sort:
    - by: date           # 可选取值：date | title | category | file | id
      order: desc        # 可选取值：desc（降序）| asc（升序）——仅影响展示序
    - by: file
      order: asc
```
...
- **章节站**：同日 `date` 时阅读序按次级键（默认 `file`）自然序，第一章的「上一篇」为更小章节号；**无需**把 order 配成 `file desc`
- **tree 分类**：目录内列表与 `content.sort` 一致（构建期对 `groups[].posts` 使用同一比较器）
- **归档 / 404**：始终日期降序；同日按 `content-index` 中的 `readingIndex`（全站阅读序位次，公开数字，无源路径）——分类内归档勿直接复用该字段
- **相关文章**：同分按 `file` 升序（与默认展示次级键一致）
...
| 配置 | 行为 |
|------|------|
| `sanitize` 省略或 `true`（默认） | DOMPurify 消毒；**消毒器加载失败时 fail-closed**（不注入，页面报错） |
| `sanitize: false` | 作者显式信任：跳过消毒，保留全部标签/属性 |

仅对「你自己编写并审核过」的 HTML 使用 `sanitize: false`。不可信来源不要关闭消毒。
...
`node serve.js` 默认开启热重载（`--no-live` 禁用），覆盖 site/ 下**全部可自定义内容**：

| 内容 | 热更新 |
|---|---|
| Markdown 文章 / 页面（.md） | ✅ |
| 自定义页面 HTML（含内联 JS） | ✅ |
| 用户主题 theme.js / theme.css / theme.yml | ✅ |
| 数据文件（content/data/*.yml、.json） | ✅ |
| config.yml 配置（含新增页面/分类/导航） | ✅ |
| 静态资源（任意扩展名，含**新建**文件/目录） | ✅ |
| 隐藏文件 / 临时文件 / node_modules | ❌ 默认忽略 |

监听规则可在 `site/config.yml` 中覆盖（默认不写死、不限制用户）：
...
本项目内置轻量级访问控制机制。**这不是真正的认证系统，是简单的密码门。**
...
auth:
  enabled: false                    # 默认关闭
  password: ""                      # 留空自动生成（保存到 site/.auth-key）
  session:
    ttl: 7200                       # 秒，-1 = 永不过期
```
...
1. 在 config.yml 中设置 `auth.enabled: true`
2. 可选：设置 `password` 或留空自动生成
3. 构建：`node build.js`
4. 首次构建时打印密钥到日志，请告知用户妥善保管
5. **不要将密码写入文章或公开内容**
