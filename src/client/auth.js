...
(function() {
  ...
  // 解锁回调：解锁成功后触发（用于重新加载依赖密码的内容，如加密文章）
  const _unlockCallbacks = [];

  function onUnlocked(fn) {
    if (typeof fn === 'function') _unlockCallbacks.push(fn);
  }
  ...
  function createAuthUI(config, onUnlock) {
    ...
    async function attemptUnlock() {
      ...
      if (hash === config.passwordHash) {
        ...
        onUnlock();
        // 通知等待密码的内容加载（如加密文章）重新执行
        _unlockCallbacks.forEach((fn) => { try { fn(); } catch (_) {} });
      } else {
  ...
  async function init() {
    ...
    if (session && session.hash === config.passwordHash) {
      // 会话有效，但缺少标签页级密码（跨标签页/新窗口打开时 sessionStorage 为空）
      // → 清除会话，要求重新输入密码，避免文章解密无密码可用
      if (!sessionStorage.getItem('blog-auth-pw')) {
        clearAuthData();
      } else {
  ...
  window.BlogAuth = { getSession, logout, lock, clearAuthData, decryptPost, onUnlocked };
})();
