...
module.exports = function sitemapPlugin(buildResult) {
  ...
  // ── 认证模式联动 ──────────────────────────────────────────
  // sitemap 暴露全部文章 URL 与存在性，认证站点不应公开该信息。
  // auth.enabled 时自动关闭并清理旧产物（增量构建场景需主动删除）。
  const auth = config.auth || {};
  if (auth.enabled) {
    const { removeTree } = require('../kernel/output');
    removeTree(path.join(distDir, 'sitemap.xml'));
    console.log('  Sitemap: skipped (auth enabled).');
  ...
  for (const [, mapping] of Object.entries(pathMap)) {
    // 收录 SSG 目录页 URL（完整文档，权威页），平铺数据页（内容片段）不收录
    urls.push({ loc: `${prefix}/${mapping.outputPath.replace(/\.html$/, '/')}`, lastmod: today, priority: '0.7' });
  }
