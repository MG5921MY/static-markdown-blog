...
这是一个零依赖的静态博客平台。2026 年，它恰好也接住了 AI 的一个新问题：AI 在帮你解决问题的过程中积累的知识锁在对话历史里，你看不到、搜不到、用不到。
...
| **开发** | 零依赖、热重载（SSE）、CLI（i18n 支持）、增量构建、自动化测试（344 项） |
...
auth:
  ...
  keepRss: false                    # 默认关闭：保留 RSS（需自行评估泄露面）
  session:
...
开启认证（`auth.enabled: true`）后，以下公开出口会自动关闭，避免绕过密码门泄露信息：

- **RSS（feed.xml）**：自动停止生成并清理旧文件——RSS 订阅器无法输入密码，保留会明文暴露文章标题/摘要/更新节奏。如确认标题/摘要不敏感，可设置 `auth.keepRss: true` 保留。
- **Sitemap（sitemap.xml）**：自动停止生成——不暴露文章 URL 与存在性。
- **收录**：robots.txt 自动改为 `Disallow: /`，页面注入 `noindex, nofollow`——认证站点不应被搜索引擎收录（爬虫无法通过密码门，收录只会得到空壳页）。
- **导航栏 RSS 按钮**：自动隐藏，避免指向已关闭的 feed 造成死链。

（文章标题/摘要仍以明文存在于 content-index.json / search-index.json 中，属首页展示与搜索所需，见下方安全边界表。）
...
| ✅ 文章正文以 AES-256-GCM 加密进入 dist/encrypted/（下载原始文件也无法读取） | ❌ 文章标题/摘要仍以明文存在于 content-index.json / search-index.json（首页展示与搜索所需），敏感信息请勿写入标题/摘要 |
...
`node serve.js` 默认开启热重载（`--no-live` 禁用），**覆盖 site/ 下全部自定义内容**：Markdown 文章/页面、自定义页面 HTML（含内联 JS）、用户主题（theme.js / theme.css / theme.yml）、数据文件、config.yml 配置、静态资源（任意扩展名）——保存即自动重建并刷新浏览器；**新增挂载（新建文章、页面、数据、图片目录）同样自动触发，无需重启**。

监听规则可在 `site/config.yml` 中覆盖（避免写死限制）：
