window.BlogRender = {
  ...
  buildDirTree(categoryId) {
    ...
    tree.posts = (category.posts || []).filter((p) => !p.groupPath);
  ...
  renderCategoryContent(categoryId, currentPath = '') {
    ...
    const html = (category.posts || []).map((post) => this.renderPostCard({
      ...post,
      category: categoryId,
      categoryName: category.name,
      categoryIcon: category.icon
  ...
  renderNavLinks() {
    ...
    const linksHtml = nav.filter((item) => this.isSafeUrl(item.url)).map((item) => {
    ...
    const footerHtml = nav.filter((item) => this.isSafeUrl(item.url)).map((item) =>
